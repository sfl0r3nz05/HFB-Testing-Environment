...
func (t *SimpleChaincode) Open(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	if len(args) != 2 {
		return shim.Error(ERROR_WRONG_FORMAT)
	}
...
func (t *SimpleChaincode) Transfer(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	if len(args) != 3 {
		return shim.Error(ERROR_WRONG_FORMAT)
	}
